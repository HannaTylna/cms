 ...
 * @return array
 */
function seven_blog_body_classes( $classes ) {
...
add_filter( 'body_class', 'seven_blog_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function seven_blog_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'seven_blog_pingback_header' );

/**
 * seven_blog Excerpt Length
 *
 * @since seven_blog 1.0.0
 *
 * @param null
 * @return void
 */

if ( ! function_exists( 'seven_blog_excerpt_length' ) ) :
   ...
   */
  function seven_blog_excerpt_length( $length ) {
...
add_filter( 'excerpt_length', 'seven_blog_excerpt_length', 999 );
